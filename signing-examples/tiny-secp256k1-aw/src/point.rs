...
/// NUM %= P
///
/// using only subtraction, made possible by 257-bit mode
#[inline]
fn fast_modulo(num: &mut BigNum, p: &BigNum) {
    while num.ugt(p) == Some(true) {
        num.sub_assign(p).unwrap();
    }
}
...
///
/// *note* it modifies B
fn mul_mod_p_assign(a: &mut BigNum, b: &mut BigNum, p: &BigNum) {
    // M = A % P
    fast_modulo(a, p);
    let mut m = inlawi!(a; ..257).unwrap();

    // A is result from now on
    a.zero_assign();

    while !b.is_zero() {
        if b.lsb() {
            // RES = (RES + M) % P
            a.add_assign(&m).unwrap();
            fast_modulo(a, p);
        }

        // B >>= 1
        b.ashr_assign(1).unwrap();
        // M <<= 1
        m.shl_assign(1).unwrap();
        // M %= P
        fast_modulo(&mut m, p);
    }
}
...
impl Point {
    ...
    */
    ...
    fn double_assign_mod(&mut self, p: &BigNum) {
        ...
        // === test of new stuff ===
        //let mut test_x = inlawi!(&self.x; ..257).unwrap();
        //let mut test_x_c = inlawi!(&self.x; ..257).unwrap();
        //mul_mod_p_assign(&mut test_x, &mut test_x_c, &p);
        //panic!("res = {}", test_x);
        // ===     =======      ===
        ...
        /*
        // int0 = (x1 * x1) % p
        let mut pad = inlawi!(0u257);
        //let mut x1_copy = inlawi!(x1; ..512).unwrap();
        let mut x1_copy = inlawi!(&self.x; ..257).unwrap();
        ...
        */
        ...
        let mut x1_1 = inlawi!(&self.x; ..257).unwrap();
        let mut x1_2 = inlawi!(&self.x; ..257).unwrap();
        mul_mod_p_assign(&mut x1_1, &mut x1_2, p);
        let mut int0 = x1_1;
        let mut pad = x1_2;
        // alive: int0, pad (2 * 33 bytes)
        ...
        */
        ...
        pad.u8_assign(3);
        mul_mod_p_assign(&mut int0, &mut pad, p);
        let mut int1 = int0;
        // alive: int1, pad (2 * 33 bytes)

        /*
        let mut y1_copy = inlawi!(self.y; ..257).unwrap();
        ...
        */
        ...
        //let mut y1_copy = inlawi!(self.y; ..257).unwrap();
        cc!(self.y; pad).unwrap();
        //y1_copy.shl_assign(1).unwrap();
        pad.shl_assign(1).unwrap();
        //let mut y1_mul2 = y1_copy;
        let mut invert_result = Self::invert(&mut pad, p);
        // alive: int1, pad, invert_result (3 * 33 bytes)
        // TODO(dave): calcualte invert result inside a separate block
        //              then cc result to `pad`, see if it improves.
        ...
        */
        ...
        mul_mod_p_assign(&mut int1, &mut invert_result, p);
        let mut lam = int1;
        // alive: lam, pad, invert_result
        ...
        */
        ...
        let mut lam_squared = invert_result; // name doesn't reflect value yet
        cc!(lam; lam_squared).unwrap();
        //lam_squared.mul_assign(&lam, &mut pad).unwrap(); // actualized
        // FIXED?
        cc!(lam; pad).unwrap();
        mul_mod_p_assign(&mut lam_squared, &mut pad, p);

        //
        // small bug.
        //
        // if this overflows, you'll get x for pk of 0x02:
        //  0xc6047f94_41ed7d6d_3045406e_95c07cd8_5c778e4b_8cef3ca7_abac09bb_5c70a687_u257
        // whereas it should be
        //  0xc6047f94_41ed7d6d_3045406e_95c07cd8_5c778e4b_8cef3ca7_abac09b9_5c709ee5_u257
        //
        if lam_squared.ult(&self.x) == Some(true) {
            lam_squared.add_assign(p).unwrap();
        }
        lam_squared.sub_assign(&self.x).unwrap(); // name doesn't reflect value anymore, again
        if lam_squared.ult(&self.x) == Some(true) {
            lam_squared.add_assign(p).unwrap();
        }
        lam_squared.sub_assign(&self.x).unwrap();
        //panic!("{lam_squared}");
        //while lam_squared.ugt(p) == Some(true) {
        //    lam_squared.sub_assign(p).unwrap();
        //}
        fast_modulo(&mut lam_squared, p);
        let x3 = lam_squared;
        // alive: lam, x3, pad
        ...
        */
        ...
        pad.add_assign(&self.x).unwrap();
        pad.sub_assign(&x3).unwrap();
        mul_mod_p_assign(&mut lam, &mut pad, p);
        lam.sub_assign(&self.y).unwrap();
        while lam.ugt(p) == Some(true) {
            lam.sub_assign(p).unwrap();
        }
    ...
    */
    ...
    fn add_assign_mod(&mut self, other: &mut Point, p: &BigNum) {
        ...
        let mut pad = inlawi!(0u257); // alloc
        cc!(Y2; pad).unwrap();
        ...
        // FIX?
        mul_mod_p_assign(&mut lam_squared, &mut lam_copy, p);
        lam_squared.sub_assign(&X1).unwrap();
        ...
        // FIX?
        mul_mod_p_assign(&mut y3, &mut lam_copy, p);
...
fn power_modulo(num: &mut BigNum, P: &BigNum) -> BigNum {
    ...
    let mut result = inlawi!(1_u257);

    let mut power = inlawi!(2_u257);
    ...
    //let mut pad = inlawi!(0_u512); // REMOVED in 257
    let mut clone = inlawi!(0_u257);
    ...
    loop {
        ...
        while !power.lsb() {
            ...
            */
            ...
            fast_modulo(num, P);
            cc!(num; clone).unwrap();
            mul_mod_p_assign(num, &mut clone, P);
        ...
        if !power.is_zero() {
            ...
            fast_modulo(&mut clone, P);
            cc!(clone; result).unwrap();
            */
            // OR.
            fast_modulo(&mut result, P);
            ...
            mul_mod_p_assign(&mut result, &mut clone, P);
